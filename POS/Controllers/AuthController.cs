...
namespace POS.Controllers
{
    public class AuthController : BaseController
    {
        ...
        [HttpPost]
        [Route("~/Pos/PasswordChange/")]
        public async Task<IActionResult> UpdateByUser([FromBody] PasswordModel PasswordModel)
        {
            ...
            try
            {
                ...
                if (!await userManager.CheckPasswordAsync(user, PasswordModel.old_password))
                {
                ...
                var result =  await userManager.ChangePasswordAsync(user,PasswordModel.old_password,PasswordModel.password1);

                if (!result.Succeeded)
                {
                    return Json(new { success = false, message = "Please write password according to rules given !" });
                }
